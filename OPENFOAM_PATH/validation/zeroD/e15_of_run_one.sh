#!/bin/bash
# Run one E15 OF job by index into e15_of_jobs.tsv
# Usage: e15_of_run_one.sh <job_index>
#
# SAFETY: only deletes under validation/zeroD/e15_conformance/of_work/<tag>_*/
set +e
...
SAFE_PREFIX="$ROOT/validation/zeroD/e15_conformance/of_work/"
...
# Resolve platform bin/lib without relying on bashrc succeeding
...
# Hard safety: never rm outside of_work/
case "$WORK" in
  "$SAFE_PREFIX"*) ;;
  *)
...
if [ -z "$WORK_REL" ] || [ "$WORK" = "$ROOT" ] || [ "$WORK" = "/" ]; then
  echo "REFUSING empty/root WORK"
  exit 99
fi
...
if ! command -v chemFoamDebug >/dev/null 2>&1; then
  echo "FATAL: chemFoamDebug not on PATH (FOAM_USER_APPBIN=$FOAM_USER_APPBIN)" | tee "$OUT/log.chemFoam"
  echo "wall_s=0" | tee "$OUT/wall.txt"
  echo "exit_code=127" | tee -a "$OUT/wall.txt"
  echo "failure=missing_binary" | tee "$OUT/failure.txt"
  exit 0
fi

echo "=== E15 OF idx=${IDX} tag=${TAG} solver=${SOLVER} endTime=${TEND} wall_cap=${WALL_CAP}s ===" | tee "$OUT/run_header.txt"
START=$(date +%s)
set +e
...
FINAL=""
for d in $(ls -1d [0-9]* 2>/dev/null | sort -g); do
  FINAL="$d"
done
if [ -n "$FINAL" ] && [ -d "$FINAL" ]; then
  ...
  for f in "$FINAL"/*; do
    ...
    case "$bn" in
      uniform|polyMesh) ;;
      *) cp -f "$f" "$OUT/fields/$bn" 2>/dev/null || true ;;
    esac
  done
...
if [ "$RC" -eq 124 ] || [ "$RC" -eq 137 ]; then
  FAIL="wall_timeout"
elif grep -q "FOAM FATAL" "$OUT/log.chemFoam" 2>/dev/null; then
  FAIL="foam_fatal"
elif grep -qE "Maximum number of iterations exceeded|JANAF" "$OUT/log.chemFoam" 2>/dev/null; then
  FAIL="thermo_newton"
elif [ ! -s "$OUT/chemFoam.out" ]; then
  FAIL="no_output"
elif ! grep -q "End" "$OUT/log.chemFoam" 2>/dev/null; then
  FAIL="incomplete"
fi
