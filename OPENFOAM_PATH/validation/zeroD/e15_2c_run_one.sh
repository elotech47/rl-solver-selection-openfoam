#!/bin/bash
# Run one E15.2c job by index (e15_2c_jobs.tsv).
set +e
...
ROOT="${ROOT:-/work}"
IDX="${1:?job index}"
TSV="$ROOT/validation/zeroD/e15_conformance/e15_2c_jobs.tsv"
SAFE_PREFIX="$ROOT/validation/zeroD/e15_conformance/e15_2c_work/"
...
echo "=== E15.2c idx=$IDX $TAG toggle=$TOGGLE solver=$SOLVER ===" | tee "$OUT/run_header.txt"
START=$(date +%s)
set +e
