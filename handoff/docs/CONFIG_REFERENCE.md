...
## Mechanism file resolution

Named keys resolve to **packaged** files under
`solver_selection_handoff/data/mechanisms/` after install
(`ndodecane`, `methane`, `jetA`). `hydrogen` uses Cantera’s built-in
`h2o2.yaml`.

Relative paths for **models and outputs** resolve against the process
working directory (or `--workdir`), not a parent research repo.
