...
| 2026-08-21 | RL `Foam::newError` / `Ss` string in libtorch under Foam                            | **ABI clash**: pre-cxx11 LibTorch + OpenFOAM + Foam `operator new`. On RHEL8 (glibc&lt;2.29) cxx11-abi zips fail (`GLIBC_2.29`). Use **ABI=0 LibTorch** + **`OFRL_POLICY_WORKER`** out-of-process (`tools/build_ofrl_policy_worker.sh`).                                                                                   |
| 2026-08-21 | Advisor: IPC vs chemistry; batching; instrument                                     | Foam already **batches** `inferBatch` per rank/τ_dec. Worker pinned `OMP/ATEN=1`. Log `policy_wall_max/sum` in `rl_usage_step.csv`. Twins must share same Opt binary; final paper timings may use in-process/hand-rolled forward.                                                                                          |
| 2026-08-21 | Twin `e18_743713` `exit=137` at t=0.05115; `Couldn't create directory processorN/…` | **LONI `/work` inode quota (4e6 files)**, not GB and not walltime. `quota` only shows `$HOME`; use **`showquota`**. Production writes a **20-field pack** every 1e-4 s (no 106 Yi). `rm -rf production/runs/e18_*/case_*/processor*` before resubmit. |
