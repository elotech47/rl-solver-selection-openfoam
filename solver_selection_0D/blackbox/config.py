...
def _resolve_mechanism(
    ...
    Accepts either:
      - ``mechanism: ndodecane`` (named key from utils.MECHANISM_CONFIGS), or
      - ``mechanism: path/to/file.yaml`` plus explicit ``fuel`` / ``oxidizer``.
    """
    # Late import so the handoff package can still be inspected without Cantera.
    from utils import MECHANISM_CONFIGS
    ...
    if named is not None:
        mechanism_path = (repo_root / named["mechanism"]).resolve()
    ...
    mechanism_path = _as_path(mech_str, repo_root)
...
def load_config(
    ...
    *,
    ...
    repo_root:
        ...
        Defaults to two directories above this file (the solver_selection root).
    """
    ...
    model_path = _as_path(
        policy.get("model_path") or raw.get("model_path"),
        config_dir if policy.get("model_path") else repo_root,
    )
    # Also try repo_root if relative path from config_dir fails later — prefer
    # explicit resolution: try config_dir first, then repo_root.
    if model_path is None:
        raise KeyError("Missing required field 'policy.model_path'.")
    if not model_path.is_file():
        alt = _as_path(policy.get("model_path"), repo_root)
        if alt is not None and alt.is_file():
            model_path = alt
        else:
            raise FileNotFoundError(
                f"RL checkpoint not found: {model_path}\n"
                f"(also checked under repo root)"
            )
    ...
    supervised_dir = _as_path(supervised.get("model_dir"), repo_root)
    ...
    out_dir = _as_path(output.get("dir", "handoff_runs/default"), repo_root)
